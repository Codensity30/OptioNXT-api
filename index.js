...
function next() {
    ...
    try {
      ...
      if (response) {
        ...
        for (let i = start; i <= end; i++) {
          ...
          if (!doc) {
            await Symbol.create({ strikePrice: sp, oiArray }).catch(
              errorHandler
            );
          } else {
        ...
        const doc = await Symbol.findOne({ strikePrice: 0 }).catch(
          errorHandler
        );
  ...
  app.get("/sp-data/:symbol/:strike", async (req, res) => {
    try {
      ...
      const data = await Sp.findOne({ strikePrice: strike }).catch(
        errorHandler
      );
